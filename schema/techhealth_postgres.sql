...
CREATE TABLE IF NOT EXISTS public.customers (
    ...
    CONSTRAINT chk_customers_subscription_type
        CHECK (subscription_type IN ('Enterprise', 'Premium', 'Basic'))
);
...
CREATE TABLE IF NOT EXISTS public.devices (
    ...
    battery_life_days             NUMERIC(3, 1) NOT NULL,
    sync_frequency_daily          INT           NOT NULL,
    active_hours_daily            NUMERIC(3, 1) NOT NULL,
    ...
    heart_rate_monitoring_enabled BOOLEAN       NOT NULL DEFAULT FALSE,
    ...
    CONSTRAINT chk_devices_active_hours_daily
        CHECK (active_hours_daily >= 0 AND active_hours_daily <= 24),

    CONSTRAINT chk_devices_battery_life_days
        CHECK (battery_life_days >= 0),

    CONSTRAINT chk_devices_device_status
        CHECK (device_status IN ('Retired', 'Inactive', 'Active')),

    CONSTRAINT chk_devices_sync_frequency_daily
        CHECK (sync_frequency_daily >= 0),

    CONSTRAINT chk_devices_total_steps_recorded
        CHECK (total_steps_recorded >= 0),

    CONSTRAINT chk_devices_total_workouts_recorded
        CHECK (total_workouts_recorded >= 0),

    CONSTRAINT fk_devices_customers
        FOREIGN KEY (user_id)
        REFERENCES public.customers (user_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS public.sales (
    ...
    CONSTRAINT chk_sales_discount_applied
        CHECK (discount_applied IS NULL OR (discount_applied >= 0 AND discount_applied <= 100)),

    CONSTRAINT chk_sales_payment_method
        CHECK (payment_method IN ('Bank Transfer', 'PayPal', 'Credit Card')),

    CONSTRAINT chk_sales_product_category
        CHECK (product_category IN ('Subscription', 'Accessory', 'Device')),

    CONSTRAINT chk_sales_quantity
        CHECK (quantity > 0),

    CONSTRAINT chk_sales_sales_channel
        CHECK (sales_channel IN ('Direct Sales', 'Retail', 'Online')),
...
CREATE TABLE IF NOT EXISTS public.healthmetrics (
    ...
    avg_deep_sleep_hours   NUMERIC(3, 1) NOT NULL,
    avg_daily_calories     INT           NOT NULL,
    avg_exercise_minutes   INT           NOT NULL,
    avg_stress_level       NUMERIC(3, 1) NULL,
    avg_blood_oxygen       NUMERIC(4, 1) NOT NULL,
    total_active_days      INT           NOT NULL,
    workout_frequency      INT           NOT NULL,
    achievement_rate       NUMERIC(3, 2) NULL,

    CONSTRAINT pk_healthmetrics PRIMARY KEY (record_id),

    CONSTRAINT chk_hm_achievement_rate
        CHECK (achievement_rate >= 0 AND achievement_rate <= 1),

    CONSTRAINT chk_hm_avg_blood_oxygen
        CHECK (avg_blood_oxygen >= 80 AND avg_blood_oxygen <= 100),

    CONSTRAINT chk_hm_avg_daily_calories
        CHECK (avg_daily_calories >= 0),

    CONSTRAINT chk_hm_avg_daily_steps
        CHECK (avg_daily_steps >= 0),

    CONSTRAINT chk_hm_avg_deep_sleep_hours
        CHECK (avg_deep_sleep_hours >= 0 AND avg_deep_sleep_hours <= 12),

    CONSTRAINT chk_hm_avg_exercise_minutes
        CHECK (avg_exercise_minutes >= 0),

    CONSTRAINT chk_hm_avg_heart_rate
        CHECK (avg_heart_rate >= 30 AND avg_heart_rate <= 200),

    CONSTRAINT chk_hm_avg_resting_heart_rate
        CHECK (avg_resting_heart_rate >= 30 AND avg_resting_heart_rate <= 100),

    CONSTRAINT chk_hm_avg_sleep_hours
        CHECK (avg_sleep_hours >= 0 AND avg_sleep_hours <= 24),

    CONSTRAINT chk_hm_avg_stress_level
        CHECK (avg_stress_level >= 0 AND avg_stress_level <= 10),

    CONSTRAINT chk_hm_total_active_days
        CHECK (total_active_days >= 0 AND total_active_days <= 31),

    CONSTRAINT chk_hm_workout_frequency
        CHECK (workout_frequency >= 0 AND workout_frequency <= 7),

    CONSTRAINT fk_healthmetrics_customers
        FOREIGN KEY (user_id)
        REFERENCES public.customers (user_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE
...
(user_id, age, gender, city, state, country, occupation, income_bracket, registration_date, subscription_type)
VALUES
...
(device_id, user_id, device_type, purchase_date, last_sync_date, firmware_version, battery_life_days, sync_frequency_daily, active_hours_daily, total_steps_recorded, total_workouts_recorded, sleep_tracking_enabled, heart_rate_monitoring_enabled, gps_enabled, notification_enabled, device_status)
VALUES
('DEV001', 'TH001', 'HealthTrack Pro', '2022-01-15', '2023-06-15', 'v3.2.1', 5.2, 24, 16.5, 2345678, 156, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV002', 'TH002', 'HealthTrack Lite', '2021-11-30', '2023-06-15', 'v3.1.0', 7.0, 12, 12.8, 1234567, 89, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV003', 'TH003', 'HealthTrack Pro', '2022-03-22', '2023-06-15', 'v3.2.1', 4.8, 24, 18.2, 1876543, 201, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV004', 'TH004', 'HealthTrack Elite', '2021-08-15', '2023-06-15', 'v3.2.1', 4.5, 48, 14.7, 3456789, 312, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV005', 'TH005', 'HealthTrack Pro', '2022-02-28', '2023-06-15', 'v3.2.1', 5.0, 24, 15.9, 2123456, 178, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV006', 'TH006', 'HealthTrack Elite', '2021-12-05', '2023-06-15', 'v3.2.1', 4.6, 48, 13.5, 4567890, 245, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV007', 'TH007', 'HealthTrack Lite', '2022-04-10', '2023-06-15', 'v3.1.0', 6.8, 12, 11.2, 987654, 67, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV008', 'TH008', 'HealthTrack Pro', '2022-01-20', '2023-06-15', 'v3.2.1', 5.1, 24, 17.3, 2789012, 198, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV009', 'TH009', 'HealthTrack Lite', '2022-05-15', '2023-06-14', 'v3.1.0', 7.2, 12, 10.5, 654321, 45, TRUE, TRUE, FALSE, TRUE, 'Inactive'),
('DEV010', 'TH010', 'HealthTrack Elite', '2021-09-30', '2023-06-15', 'v3.2.1', 4.4, 48, 16.8, 3901234, 289, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV011', 'TH011', 'HealthTrack Pro', '2022-03-01', '2023-06-15', 'v3.2.1', 5.3, 24, 15.4, 2345678, 167, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV012', 'TH012', 'HealthTrack Lite', '2021-10-15', '2023-06-15', 'v3.1.0', 6.9, 12, 11.8, 1123456, 78, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV013', 'TH013', 'HealthTrack Pro', '2022-02-14', '2023-06-15', 'v3.2.1', 4.9, 24, 16.7, 2567890, 187, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV014', 'TH014', 'HealthTrack Elite', '2021-11-01', '2023-06-15', 'v3.2.1', 4.7, 48, 15.2, 4123456, 298, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV015', 'TH015', 'HealthTrack Lite', '2022-06-01', '2023-06-13', 'v3.1.0', 7.1, 12, 9.8, 456789, 34, TRUE, TRUE, FALSE, TRUE, 'Inactive'),
('DEV016', 'TH016', 'HealthTrack Pro', '2022-01-05', '2023-06-15', 'v3.2.1', 5.0, 24, 16.1, 2234567, 165, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV017', 'TH017', 'HealthTrack Lite', '2022-04-22', '2023-06-15', 'v3.1.0', 6.7, 12, 11.5, 876543, 56, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV018', 'TH018', 'HealthTrack Pro', '2021-12-15', '2023-06-15', 'v3.2.1', 5.2, 24, 17.8, 2901234, 211, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV019', 'TH019', 'HealthTrack Lite', '2022-03-30', '2023-06-15', 'v3.1.0', 7.0, 12, 10.9, 765432, 43, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV020', 'TH020', 'HealthTrack Pro', '2021-10-30', '2023-06-15', 'v3.2.1', 5.1, 24, 15.6, 2445678, 177, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV021', 'TH021', 'HealthTrack Lite', '2022-02-01', '2023-06-15', 'v3.1.0', 6.9, 12, 11.1, 998765, 69, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV022', 'TH022', 'HealthTrack Elite', '2021-09-15', '2023-06-15', 'v3.2.1', 4.5, 48, 16.4, 3789012, 267, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV023', 'TH023', 'HealthTrack Pro', '2022-05-01', '2023-06-15', 'v3.2.1', 5.0, 24, 15.8, 2223456, 156, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV024', 'TH024', 'HealthTrack Elite', '2021-11-15', '2023-06-15', 'v3.2.1', 4.6, 48, 14.9, 4234567, 301, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV025', 'TH025', 'HealthTrack Lite', '2022-06-15', '2023-06-14', 'v3.1.0', 7.2, 12, 9.5, 543210, 38, TRUE, TRUE, FALSE, TRUE, 'Inactive'),
('DEV026', 'TH026', 'HealthTrack Pro', '2022-01-10', '2023-06-15', 'v3.2.1', 5.3, 24, 16.9, 2678901, 189, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV027', 'TH027', 'HealthTrack Pro', '2022-03-15', '2023-06-15', 'v3.2.1', 4.9, 24, 17.1, 2456789, 182, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV028', 'TH028', 'HealthTrack Elite', '2021-08-30', '2023-06-15', 'v3.2.1', 4.4, 48, 15.7, 4012345, 278, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV029', 'TH029', 'HealthTrack Lite', '2022-04-05', '2023-06-15', 'v3.1.0', 6.8, 12, 11.3, 887654, 58, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV030', 'TH030', 'HealthTrack Pro', '2021-12-20', '2023-06-15', 'v3.2.1', 5.1, 24, 16.2, 2567890, 188, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV031', 'TH031', 'HealthTrack Pro', '2022-02-15', '2023-06-15', 'v3.2.1', 5.2, 24, 15.9, 2345678, 171, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV032', 'TH032', 'HealthTrack Elite', '2021-10-01', '2023-06-15', 'v3.2.1', 4.7, 48, 14.8, 3987654, 289, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV033', 'TH033', 'HealthTrack Lite', '2022-05-20', '2023-06-15', 'v3.1.0', 7.1, 12, 10.7, 654321, 47, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV034', 'TH034', 'HealthTrack Pro', '2021-11-20', '2023-06-15', 'v3.2.1', 5.0, 24, 16.6, 2789012, 198, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV035', 'TH035', 'HealthTrack Pro', '2022-03-10', '2023-06-15', 'v3.2.1', 4.8, 24, 17.4, 2567890, 187, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV036', 'TH036', 'HealthTrack Elite', '2021-09-01', '2023-06-15', 'v3.2.1', 4.5, 48, 15.1, 4234567, 299, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV037', 'TH037', 'HealthTrack Lite', '2022-06-10', '2023-06-14', 'v3.1.0', 7.0, 12, 9.9, 567890, 41, TRUE, TRUE, FALSE, TRUE, 'Inactive'),
('DEV038', 'TH038', 'HealthTrack Pro', '2022-01-25', '2023-06-15', 'v3.2.1', 5.1, 24, 16.3, 2456789, 176, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV039', 'TH039', 'HealthTrack Pro', '2022-04-15', '2023-06-15', 'v3.2.1', 5.0, 24, 15.7, 2345678, 169, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV040', 'TH040', 'HealthTrack Elite', '2021-08-01', '2023-06-15', 'v3.2.1', 4.6, 48, 14.6, 4123456, 295, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV041', 'TH041', 'HealthTrack Lite', '2022-05-05', '2023-06-15', 'v3.1.0', 6.9, 12, 11.4, 876543, 57, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV042', 'TH042', 'HealthTrack Elite', '2021-12-10', '2023-06-15', 'v3.2.1', 4.4, 48, 15.3, 3901234, 276, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV043', 'TH043', 'HealthTrack Pro', '2022-02-20', '2023-06-15', 'v3.2.1', 5.2, 24, 16.8, 2567890, 184, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV044', 'TH044', 'HealthTrack Pro', '2021-10-20', '2023-06-15', 'v3.2.1', 5.1, 24, 15.5, 2678901, 191, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV045', 'TH045', 'HealthTrack Lite', '2022-03-25', '2023-06-15', 'v3.1.0', 7.2, 12, 10.8, 765432, 49, TRUE, TRUE, FALSE, TRUE, 'Active'),
('DEV046', 'TH046', 'HealthTrack Pro', '2022-01-30', '2023-06-15', 'v3.2.1', 4.9, 24, 17.2, 2456789, 179, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV047', 'TH047', 'HealthTrack Pro', '2022-04-20', '2023-06-15', 'v3.2.1', 5.0, 24, 16.4, 2567890, 186, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV048', 'TH048', 'HealthTrack Elite', '2021-11-10', '2023-06-15', 'v3.2.1', 4.5, 48, 15.0, 4012345, 287, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV049', 'TH049', 'HealthTrack Lite', '2022-06-05', '2023-06-14', 'v3.1.0', 7.1, 12, 10.2, 654321, 44, TRUE, TRUE, FALSE, TRUE, 'Inactive'),
('DEV050', 'TH050', 'HealthTrack Pro', '2021-12-25', '2023-06-15', 'v3.2.1', 5.3, 24, 16.0, 2789012, 197, TRUE, TRUE, TRUE, TRUE, 'Active'),
('DEV051', 'TH016', 'HealthTrack Pro 2', '2025-05-08', '2025-05-08', 'v4.0.0', 8.0, 12, 10.0, 0, 0, TRUE, TRUE, FALSE, TRUE, 'Active');

-- NOTE: Sales + HealthMetrics inserts are long; they convert 1:1 (no dialect conflicts).
-- If you want, I can output them in a second block to keep messages shorter.
...
(sale_id, user_id, sale_date, product_id, product_name, product_category, unit_price, quantity, discount_applied, total_amount, payment_method, subscription_plan, sales_channel, region, sales_rep_id)
VALUES
...
('S051', 'TH026', '2025-04-25', 'SUB-001', 'Premium Subscription', 'Subscription', 14.99, 12, 10.00, 161.89, 'Credit Card', 'Premium', 'Online', 'West', 'REP003'),
('S052', 'TH026', '2025-04-25', 'PRO-001', 'HealthTrack Pro', 'Device', 299.99, 1, 5.00, 284.99, 'Credit Card', 'Premium', 'Online', 'West', 'REP003'),
('S053', 'TH026', '2025-04-25', 'ACC-001', 'Sports Band', 'Accessory', 29.99, 1, 0.00, 29.99, 'Credit Card', 'Premium', 'Online', 'West', 'REP003'),
('S054', 'TH035', '2025-04-28', 'SUB-001', 'Premium Subscription', 'Subscription', 14.99, 12, 10.00, 161.89, 'Credit Card', 'Premium', 'Retail', 'Europe', 'REP005'),
('S055', 'TH035', '2025-04-28', 'LIT-001', 'HealthTrack Lite', 'Device', 199.99, 1, 20.00, 159.99, 'Credit Card', 'Premium', 'Retail', 'Europe', 'REP005'),
('S056', 'TH036', '2025-05-01', 'SUB-003', 'Enterprise Subscription', 'Subscription', 24.99, 12, 15.00, 254.90, 'Bank Transfer', 'Enterprise', 'Retail', 'West', 'REP003'),
('S057', 'TH038', '2025-05-01', 'SUB-001', 'Premium Subscription', 'Subscription', 14.99, 12, 10.00, 161.89, 'Credit Card', 'Premium', 'Direct Sales', 'South', 'REP004'),
('S058', 'TH038', '2025-05-01', 'PRO-001', 'HealthTrack Pro', 'Device', 299.99, 1, 5.00, 284.99, 'Credit Card', 'Premium', 'Direct Sales', 'Northeast', 'REP002'),
('S059', 'TH039', '2025-05-02', 'PRO-001', 'HealthTrack Pro', 'Device', 299.99, 1, 0.00, 299.99, 'Credit Card', 'Premium', 'Online', 'Europe', 'REP005'),
('S060', 'TH039', '2025-05-05', 'SUB-001', 'Premium Subscription', 'Subscription', 14.99, 12, 10.00, 161.89, 'Credit Card', 'Premium', 'Online', 'Europe', 'REP005'),
('S061', 'TH040', '2025-05-05', 'PRO-001', 'HealthTrack Pro', 'Device', 299.00, 1, 0.00, 299.00, 'Bank Transfer', 'Enterprise', 'Retail', 'West', 'REP003');


INSERT INTO public.healthmetrics
(record_id, user_id, month_date, avg_heart_rate, avg_resting_heart_rate, avg_daily_steps, avg_sleep_hours, avg_deep_sleep_hours, avg_daily_calories, avg_exercise_minutes, avg_stress_level, avg_blood_oxygen, total_active_days, workout_frequency, achievement_rate)
VALUES
